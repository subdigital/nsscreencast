...
class ViewController: UITableViewController {
    ...
    func onDownloadProgress(notification: NSNotification) {
        ...
        guard let episode = getEpisode(byID: episodeId) else { return }
        
        progressValues[episode] = progress
        
        guard let indexPath = indexPath(for: episode) else { return }
        
        if tableView.indexPathsForVisibleRows?.contains(indexPath) == true {
            print("reloading row \(indexPath.row)")
    ...
    func getEpisode(byID id: Int) -> Episode? {
        return fetchedResultsController?.fetchedObjects?.filter({
            return $0.id == Int32(id)
        }).first
    }
    
    func indexPath(for episode: Episode) -> IndexPath? {

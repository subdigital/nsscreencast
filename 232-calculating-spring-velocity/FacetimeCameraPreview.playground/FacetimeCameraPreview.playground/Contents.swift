...
class Handler: NSObject, UIGestureRecognizerDelegate {
    ...
    @objc func onPan(recognizer: UIPanGestureRecognizer) {
        ...
        case .Ended:
            ...
            let distanceVector = CGPoint(x: centerInParent.x - targetPoint.x,
                                         y: centerInParent.y - targetPoint.y)
            let distance = magnitude(distanceVector)
            let magVelocity = magnitude(velocity)

            let relativeDistance = magVelocity / distance
            
            let animationDuration: NSTimeInterval = 1
            let springVelocity: CGFloat = relativeDistance / CGFloat(animationDuration)

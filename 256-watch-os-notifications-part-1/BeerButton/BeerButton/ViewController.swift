...
class ViewController: UITableViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate, WCSessionDelegate, NotificationPromptDelegate {
    ...
    let promptView = Bundle.main.loadNibNamed("NotificationPrompt", owner: nil, options: nil)!.first as! NotificationPrompt
    ...
    override func viewDidLoad() {
        ...
        updateWatchBeers()
        promptUserForNotificationPermissions()
    }
    ...
    func promptUserForNotificationPermissions() {
        let center = UNUserNotificationCenter.current()
        center.getNotificationSettings { (settings) in
            if settings.authorizationStatus == .notDetermined {
                DispatchQueue.main.async {
                    self.setUpNotificationPromptView()
                }
            }
        }
    }
...
extension ViewController {
    ...
    func setUpNotificationPromptView() {
        ...
        navView.addSubview(promptView)
        promptView.centerYAnchor.constraint(equalTo: navView.centerYAnchor).isActive = true
        promptView.centerXAnchor.constraint(equalTo: navView.centerXAnchor).isActive = true
        promptView.leadingAnchor.constraint(equalTo: navView.leadingAnchor, constant: 15.0).isActive = true
        promptView.trailingAnchor.constraint(equalTo: navView.trailingAnchor, constant: -15.0).isActive = true
        promptView.heightAnchor.constraint(equalToConstant: 150).isActive = true
    }
}

...
struct FakeHealthData {
    static func randomSamplesWithNoStreaks(calendar calendar: NSCalendar, startingWithDate date: NSDate) -> [ActivityLog] {
        let randomSamples: [ActivityLog] = (0..<25).map { i in
            
            let date = calendar.dateByAddingUnit(.Day, value: -i, toDate: date, options: [])!
            let log = ActivityLog.randomSampleWithDate(date,
                moveCompleted: false,
                exerciseCompleted: false,
                standCompleted: false)

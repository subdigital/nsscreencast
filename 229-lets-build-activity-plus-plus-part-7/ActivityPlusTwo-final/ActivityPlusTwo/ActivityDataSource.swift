...
struct ActivityDataSource {
    ...
    lazy var streaks: [Streak] = {
        ...
        var lookup: [MetricType : (ActivityLog) -> Bool ] = [
            .Movement : { $0.activityProgress > 0 },
            .Exercise : { $0.exerciseProgress > 0 },
            .Standing : { $0.standProgress > 0 }
        ]
        
        for log in self.activities {
            for metric in lookup.keys {
                let hasCompletedMetric = lookup[metric]!(log)
                if hasCompletedMetric {
                    ...
                    if let numberOfDays = potentialStreaks[metric] where numberOfDays > 1 {
                        streaks.append(Streak(metricType: metric, numberOfDays: numberOfDays))
                    }
